...
### 11.4 구현 상태 (2026-09-03, 8주차 끝 = 2단계 종료)
공지 사진(3장, Storage 비공개·서명 URL) ✓ · 학원 로고 업로드(공개 버킷, 문·인증 화면) ✓ · 아직 안 들어온 사람 명단·초대 다시 보내기 ✓ · 숙제 검사(원장 대신 체크) ✓ · 반별 출결표 CSV ✓.

## 13. 3단계로 넘기는 것 (2026-09-03)

2단계 8주가 끝났다. §8 표의 1~4주 구축과 5~8주 파일럿·여유분 중 **코드로 할 수 있는 것은 전부 끝났고**, 남은 것은 사용자 쪽 관문(원장 파일럿 시작·실제 폰 확인·도메인·대행사 계약·템플릿 심사)과 아래 3단계 목록이다. 3단계는 파일럿 피드백이 들어온 뒤 시작한다.

| 항목 | 왜 3단계 | 준비된 것 |
|---|---|---|
| 수강료(청구·납부 확인·미납 안내) | 원장의 규칙(할인·일할·CMS) 답이 필요 | §12 스키마(`0003_billing.sql`)·흐름·템플릿 3개 |
| 시험 점수·성적 추이 / 성장 기록 | 채점 체계를 먼저 받아야 함 | 더보기 "준비 중" 자리 |
| 첨삭·편지 | 파일럿 뒤 원장 관심도 확인 | 더보기 "준비 중" 자리 |
| 로고 → 매니페스트 아이콘·앱바 흰 로고 | 학원마다 아이콘 빌드가 필요(지금은 파일럿 학원 것) | `academies.logo_path`, 문 화면 로고 |
| 학원별 알림톡 발신키·채널 | `[#{학원}]` 단일 템플릿 모델을 대행사가 허용하는지에 달림 | `ALIMTALK_SENDER_KEY` 전역 |
| 보호자 여럿 우선순위·알림 설정 세분화 | 파일럿에서 요구가 나오면 | `guardians`, `users.prefs` |
| 예약 공지·공지 삭제(사진 정리 포함) | 삭제 흐름이 아직 없음(`removeNoticePhotos` 는 있음) | Storage 정책 |
| 월 보고서 PDF | CSV 로 충분한지 파일럿에서 확인 | 출결표 CSV |
| GitHub Pages Action 배포 | 저장소에 `pwa/` 를 커밋하는 방식이 회당 ~1MB 쌓임 | `deploy.mjs` |
