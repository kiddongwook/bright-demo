...
- DB `0013_push_invite` 적용, VAPID 키 생성 → `.env.local`·Edge 비밀값(`VAPID_*`, 개발 중 `PUSH_DRY_RUN=1`), `outbox-send`·`otp-verify`·`invite-login` 배포. 앱 빌드에 공개키(`VITE_VAPID_PUBLIC`) 포함 확인, `pwa/push-sw.js` 와 `sw.js` 의 importScripts 확인.
- 회귀: otp · invite A~C · push A~G+H(드라이런) · outbox A~F · manage A~K · onboard · rls 모두 PASS. 단위 56/56.
- 헤드리스 UI: 학부모 초대 링크 → 새 기기에서 바로 학부모 홈(`?i=` 제거, `?a=` 유지) → 같은 링크 10분 안 재사용 OK → 11분 뒤 "이미 사용한 링크예요" → 이미 들어와 있는 기기에서 다른 초대 링크는 "이미 들어와 있어요 · 지금 계정 그대로 / 초대 링크로 새로 들어가기" 로 묻는다(Fable 이 추가). 알림 설정 화면은 헤드리스에서 "브라우저에서 막았어요" 상태를 바르게 보여줌.
- 아직 못 본 것: **실제 기기 푸시 수신**. `PUSH_DRY_RUN=1` 을 지우고(`npx supabase secrets unset PUSH_DRY_RUN`) 사용자가 폰에서 설치 → 알림 설정 → "이 기기로 알림 받기" 켠 뒤 공지 하나를 올려 확인해야 한다. 안드로이드 배지 아이콘은 흰 실루엣으로 보일 수 있음(단색 아이콘은 추후).
- 메모: 초대 링크는 사용자 생성까지 하므로 명부에 있는 번호면 누구나 첫 로그인이 가능. 원장 첫 초대는 `tools/new-academy.mjs` 가 링크를 출력.
