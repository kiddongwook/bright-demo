...
- `outbox-test` A~E PASS (트리거 매핑 5종 → 발송 5건·토큰 5개 → 콜백 실패→문자 대체 → 5회 실패→dead→문자 → 링크 로그인·만료·재사용·기존 세션 유지). `flow-test`·`rls-test`·`otp-test` PASS, 단위 13/13, 빌드 OK.
- 1분 틱: 씨앗 학원에 공지를 올리고 손대지 않았더니 55초 뒤 5건 sent (pg_cron → pg_net → outbox-send). `create extension pg_cron/pg_net` 은 `db push` 로 바로 됐다.
- 브라우저(Playwright 390×844): 알림톡 URL 을 새 컨텍스트에서 열면 "문을 여는 중" → 공지 상세, 아래 "카톡에서 열었어요 · 전체 기능은 번호로 들어가기", 탭바·종 없음 → 누르면 문(Gate), 세션 삭제. 새 버전 배너: 재빌드 뒤 0.5초 안에 뜨고 새로고침으로 새 번들. 설치 안내: PC 문구, 카톡 UA 에선 "브라우저로 열기"(`kakaotalk://web/openExternal`). 시트 `tests/app-verify-week3.png`.
- 계획에서 바꾼 것
  - `link-login` 은 비밀번호를 갈지 않고 **매직링크 생성 → verifyOtp** 로 세션을 만든다. 알림톡을 누를 때마다 설치된 앱의 세션이 끊기면 안 되므로. 테스트 E 가 기존 세션의 리프레시가 사는지 확인한다.
  - 제한 세션에선 종(알림) 단추도 숨긴다 — 알림 목록을 거쳐 다른 화면으로 나가는 길을 막는다.
  - `outbox-callback` 은 헤더 `X-Outbox-Key` 또는 쿼리 `?key=` 둘 다 받는다(대행사가 헤더를 못 붙이는 경우).
  - 새 버전 배너: 첫 방문 뒤 첫 갱신은 페이지가 서비스워커 관할이 아니라 `controllerchange` 가 안 온다 → 그땐 직접 `location.reload()`.
  - 테스트는 도는 동안 `app_settings.outbox_url` 을 빼서 틱이 끼어들지 않게 한다.
- 남긴 것(4주차): 실제 폰에서 카톡 내장 브라우저의 `pagehide` 가 앱 전환에도 오는지 확인(오면 제한 세션이 너무 일찍 끝난다 — 토큰은 7일 재사용이라 복구는 된다). 자녀 둘인 학부모의 `child:` 링크는 활성 자녀 화면이 열린다 — 링크의 ref 로 자녀를 골라 활성 소속을 바꾸는 한 줄. 화이트라벨(`slug` 로 학원 식별). `app_settings` → Vault.
