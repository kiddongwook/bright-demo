...
## 검증 결과 — Task A (2026-09-03)

- 빈 학원(임시 `uiux-test`, 검증 뒤 삭제) 원장: 오늘 탭에 첫걸음 카드(반 만들기 → 학생·학부모 넣기 → 초대 문구 → 첫 공지) 만 보이고 출석부·저장·결석 신청은 숨김. "가기" 가 반·시간표로 이동, 반 없음 → `Empty` + 반 추가. 공지·문의·알림 빈 상태 확인.
- 씨앗 학원 원장: 오늘 요약 타일(오늘 수업 1개 · 기록 1/1, 답변 대기 1, 결석 신청 1) 폰·다크 확인. 학부모 홈 순서(다음 수업 → 이번 주 → 할 것 → 최근 공지 1건 → 미리 알린 결석) 다크 확인.
- 고친 것: 반이 0개일 때 출석부 블록 숨김(Today), 같은 화면에 단추가 이미 있는 빈 상태에서는 CTA 제거(공지·명부·반).
- 단위 31/31, tsc, build 통과. 다크 테마는 별도 커밋(37f751e)으로 먼저 배포.

## 검증 결과 — Task B (2026-09-03)

- 확인 시트: 학생 편집 → 퇴원 처리에서 앱 안 하단 시트(제목·설명·취소·빨간 퇴원 처리) 확인, 취소하면 아무 일 없음. 네이티브 `confirm()` 은 화면 코드에서 사라짐(퇴원·강사 빼기·로고 지우기·재입학은 시트, 할 것·휴원일·메모·FAQ 삭제는 되돌리기).
- 되돌리기: 할 것 지우기 → 토스트 "「…」을 지웠어요 · 되돌리기" → 누르면 목록에 복귀(전후 목록 비교로 확인). 토스트가 좁게 줄바꿈되던 것은 `width:max-content; max-width:min(420px, 100vw-40px)` 로 고침.
- 폼·터치: 입력칸 48px·포커스 링, 행 56px, 세그먼트 44px 확인. 아이콘: 앱바 뒤로가기·더보기 내려받기/복사·공지 사진·명부 완료 표시가 SVG 로 바뀜.
- 스켈레톤·오류 화면은 코드 검토로 확인(빈 `view on` 섹션 0곳). 단위 31/31, tsc, build 통과.
- 검증 중 씨앗 데이터 "영작 1편 — My Summer Plan" 할 것을 지웠다가 다시 넣음(학생 체크 1건은 사라짐).

## 검증 결과 — Task C (2026-09-03)

- PC 1280: 원장 세션에서 `body.pc` 켜지고 좌측 내비(탭 4개 + 관리 6개 + 빌드 스탬프), 본문 960px 가운데, 하단 탭 숨김. 폰 390 에서는 변화 없음(`body.pc` 없음, 하단 탭 그대로). 앱바 로고가 좌측 내비와 겹쳐 PC 모드에서는 앱바 로고를 감춤.
- 공지 쓰기: 템플릿 칩(휴원 안내) → 제목·본문 채움 → 대상 고2 B → 미리보기(학부모 화면과 같은 `NoticeBody`) → "6명에게 알림이 가요" 확인. 대상은 데이터 모델대로 단일 선택(전체 또는 반 하나). 인원은 `entryStatus`(원장 전용) 로 학생·학부모 번호를 중복 없이 셈 — 강사에게는 이 줄이 보이지 않음.
- 단위 35/35, tsc, build 통과.
- 검증 중 사고: B 검증 때 확인 시트의 "퇴원 처리" 가 눌려 씨앗 학생 김민수가 퇴원 처리됨 → 재입학 시트로 되돌리고 반·번호 다시 넣음(출결 기록은 그대로).
- 화면 시트(`tests/app-verify-uiux.png`)는 만들지 않음 — 브라우저 패널 스크린샷은 파일로 남지 않아 결과를 이 절에 글로 남김.

## 마무리

A·B·C 모두 배포됨(커밋 b80a9d9 · eb3d248 · 이 커밋). 남은 관찰: 스켈레톤·오류 화면은 코드 검토로만 확인했으니 실기기에서 느린 네트워크로 한 번 볼 것; 날짜·시간 입력칸은 아이폰에서 패딩이 달라질 수 있음.

## 추가 — 시각 시스템 정리 (2026-09-03, 사용자 피드백 "버튼이 붙어 있음 · 블루 테마가 올드함")

- 기본 강조색 `#2B5BD9` → `#3182F6`, 연한 강조색은 강조색에서 자동 생성(`softFor`, 밝음 12%·어두움 22% 섞기). 강조색 칩 5종 교체(파랑·먹색·초록·주황·보라). 매니페스트 `theme_color` 도 새 파랑. 기본값 그대로였던 `yeongeo`·`yeongeo-jip` 의 brand_color 를 새 기본값으로 옮김(DB 기본값 마이그레이션은 그대로).
- 파랑은 주 버튼·활성 탭·선택 표시·포커스 링에만. 테두리 버튼·세그먼트(iOS 식 흰 알약)·아바타 원·내 말풍선·달력 화살표·FAQ Q 는 먹색·회색. 카드는 그림자 없이 16px·얇은 테두리. 바탕·선 색 살짝 차갑게.
- 버튼 간격: `.btnrow + .btnrow` 10px. 학생 상세의 탭 줄에 있던 "편집" 은 이름 옆으로, "다시 다니기" 는 탭 아래 단추로.
- 폰(밝음·어두움) 오늘·학생 상세 확인. 단위 35/35, tsc, build 통과.

## 추가 — 주 버튼 먹색화 · 전 화면 밝음/어두움 전수 점검 (2026-09-03)

- 사용자 피드백 "아직도 블루 버튼": 주 버튼(`.btn`) 을 `--inverse`(밝음 먹색 · 어두움 흰색) 로. 파랑은 활성 탭·선택 표시·링크·포커스 링·오늘 테두리에만 남음. `.btn:disabled` 45% 투명.
- 수동 테마: `localStorage.theme = 'dark'|'light'` → `html[data-theme]`, CSS 는 `:root:not([data-theme=light])` 미디어 블록 + `:root[data-theme=dark]` 복제 블록. 지금은 점검용이고 설정 화면 스위치는 아직 없음(3단계 후보).
- 전수 점검: `scratchpad/sweep/sweep.mjs`(playwright-core + 헤드리스 크로미움) 로 원장 15 · 학부모 7 · 학생 3 화면 × 밝음/어두움 = 50장. 시트 `tests/app-verify-uiux-{director,parent,student}-{light,dark}.png`. 두 테마에서 깨지는 곳 없음(토큰만 쓴 결과). 점검 스크립트의 "강사" 행 매칭이 반·시간표로 빗나가 강사 화면은 이전 검증(다크 명부·강사) 로 갈음.
- 관찰: 학생 편집의 반 선택(`.seg.col`) 이 입력칸과 모양이 비슷함 — 칩으로 바꾸는 것 후보. 알림 종의 숫자는 알림 화면을 다녀오면 0 이 되는 정상 동작.
