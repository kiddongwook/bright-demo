...
빌드가 넣어 준 정적 매니페스트(영어의 집)는 첫 페인트용 대비책으로 남는다.

- 아이콘은 원장님이 **더보기 → 우리 학원**에서 올린 로고다(정사각 512×512 PNG 로 줄여서 `logos` 버킷에 저장).
  로고가 없으면 기본 아이콘. 이름은 학원 이름, 색은 강조색.
...
4. **카톡 링크 미리보기(og:image·og:title)는 계속 "영어의 집"** 이다 — 소개 페이지의 og 태그는 정적 파일이라
...
같은 `public_academy` 를 불러 이름·강조색·로고를 갈아 끼우고, 오른쪽 아래 **「앱 열기」** 가 `pwa/?a=<slug>` 로 간다.
`?a=` 없이 열면 지금까지의 영어의 집 데모 그대로다(모르는 slug 도 데모로 남는다).
