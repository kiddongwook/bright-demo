...
## 7. 방향 결정과 적용 (2026-09-04)

- 사용자가 색 실험실(A 등불 · B 네이비 칠판 · C 색 블록 · D 혼합)에서 **C · 색 블록**을 골랐다. 흰 바탕, 회색 카드(`#F5F6FA`, 테두리 없음), 코발트 `#2F5BEA`, 주 버튼·학부모 다음 수업 카드는 코발트→보라 그라디언트, 요약 타일은 하늘·복숭아·민트, 탭바는 먹색 알약, 모서리 20/14/8/24. 어두운 테마는 반전이 아니라 따로 설계(`#0F1115` 바탕, `#1A1D24` 카드, 타일 `#1E2A44/#3A2A22/#1D3328`).
- 바탕/카드 반전(흰 바탕 + 회색 카드)으로 깨지는 곳을 손봄: 구분선 `--rule` 한 단계 진하게, 카드 안의 테두리 버튼·입력칸은 흰색, 원형 표시는 흰 채움, 눌림 상태는 `--rule`.
- **출석부**를 이름 옆 ○△✕ 에서 **학생 타일**(3열, PC 6열)로: 누를 때마다 미기록 → 출석 → 지각 → 결석. 상태는 의미색(초록·호박·빨강 `--att-*`)으로 타일 전체가 물든다. "전원 출석"(모두 출석이면 "전체 지우기") 과 아래 요약 줄 "출석 N · 지각 N · 결석 N · 미기록 N".
- 사용자 UX 지적 반영: ① 탭 루트가 아닌 화면(공지 쓰기·학생 편집·결석 알리기 등)에서는 탭바를 없애고 바닥에 취소·저장 CTA만(`body.no-tab`). ② 오늘 탭의 "저장하고 알리기"는 출석 표시가 바뀐 때만 슬라이드업으로 나타나고 저장 뒤 사라짐(`dirty` = 서버 상태와 다른 학생이 하나라도 있음). ③ 요약 타일 바로가기는 이미 있었고 `›` 와 눌림 상태를 더함. ④ 하위 화면 앱바의 종 숨김. 임시저장은 3단계 후보.
- 확인: 폰 밝음/어두움 55장 + PC·프레임 6장. 시트 `tests/app-verify-c-*.png`. 고친 것: 어두운 테마에서 다음 수업 카드 부제 색.
- 데이터: 기본값 그대로였던 `yeongeo`·`yeongeo-jip` 의 brand_color 를 `#2F5BEA` 로.

## 8. 아이콘 마무리 · 뒤로가기 제스처 (2026-09-04)

- 더보기 모든 행에 앞 아이콘(36px 둥근 상자, `--avatar` 색; 준비 중 행은 회색), 명부 "아직 안 들어온" 목록의 번호는 전화 링크 + 수화기, 학생 상세 타임라인·메모 배지에 종류별 아이콘.
- 뒤로가기: 내비 상태(`{tab, stack}`)를 `history.pushState` 로 미러링(`lib/nav-history.ts`). 화면을 열거나 탭을 바꾸면 히스토리 한 칸, 제스처·브라우저 뒤로가기는 `popstate` 로 이전 스냅샷 적용, 코드의 `back()` 도 `history.back()` 을 거쳐 같은 길. 확인 시트도 한 칸을 차지해 제스처로 닫힘. 세션이 바뀌면 이전 항목은 루트로 취급(`k < base`). 탭 루트에서 더 뒤로 가면 플랫폼 동작(앱 종료).
- 헤드리스 확인: 더보기 → 명부 → 학생 → 뒤로 ×3 = 명부 → 더보기 → 오늘; 학생 편집에서 퇴원 시트 열고 뒤로 → 시트만 닫힘; 취소 단추 → 명부. 단위 40/40.
