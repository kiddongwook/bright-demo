...
import { myChildren, weekAttendance, listTodos, listAbsences, requestAbsence, weekRange, nextClassDays, fmtMDW, DOW, dowOf, type Student, type Todo, type Absence, type AttStatus } from '../../lib/api';
...
export function WeekStrip({ studentId, absences }: { studentId: string; absences: Absence[] }) {
  ...
  const cls = (d: string) => {
    ...
    if (absences.some(a => a.student_id === studentId && a.makeup_at && a.makeup_at.slice(0, 10) === d)) return 'm';
    return '';
  };
...
export function Child() {
  ...
  return (
    <section className="view on">
      <div className="head"><h1 className="hello">{short}이</h1><p className="lede">{child.classes.map(c => c.name).join(' · ')}{nextCls ? ` · 다음 수업 ${fmtMDW(next)} ${nextCls.schedule.find(s => s.dow === dowOf(next))?.start ?? ''}` : ''}</p></div>
      ...
      {mine.length ? <div className="box">{mine.map(a => <div key={a.id} className="rw" style={{ cursor: 'default' }}><span className="bd"><span className="t">{fmtMDW(a.date)} 결석</span><span className="s">{a.reason}</span></span>
        {a.status === 'requested' ? <span className="tag warn">원장님 확인 중</span> : a.status === 'declined' ? <span className="tag muted">보류</span> : <span className="tag ok">{a.makeup_kind === 'material' ? '자료로 대체' : '보강 ' + (a.makeup_at ? fmtMDW(a.makeup_at.slice(0, 10)) : '')}</span>}</div>)}</div>
