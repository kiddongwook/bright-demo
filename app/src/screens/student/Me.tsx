...
export function Me() {
  ...
  return (
    <section className="view on">
      <div className="head"><h1 className="hello">{me.name.replace(/^[가-힣]/, '')}</h1><p className="lede">{fmtMDW(kstToday())} · {me.classes.map(c => c.name).join(' · ')}{nextCls ? ` · 다음 수업 ${fmtDayOrToday(next)} ${nextCls.schedule.find(s => s.dow === dowOf(next))?.start ?? ''}` : ''}</p></div>
