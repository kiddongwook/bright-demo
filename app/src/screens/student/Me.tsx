...
export function Me() {
  ...
  const load = () => { if (!me) return; listTodos(me.classes.map(c => c.id), me.id).then(setTodos).catch(errToast); listAbsences().then(setAbsences).catch(errToast); };
  useEffect(load, [me?.id]);
  if (!me) return <section className="view on" />;
  const left = todos.filter(t => !t.done).length;
  const next = nextClassDays(me.classes.flatMap(c => c.schedule ?? []), 1)[0];
