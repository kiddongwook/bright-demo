/* 학원별 설치 정체성 — 한 PWA 가 여러 학원을 서빙하므로, 홈 화면에 놓이는 이름·아이콘·색이 ?a=<slug> 에 따라 달라야 한다.
   ...
   빌드가 넣어 준 정적 매니페스트(제품 기본값 BRIGHT)는 첫 페인트용 대비책으로 index.html 에 그대로 둔다. */
...
export function buildManifest(i: IdentityInput): WebManifest {
  ...
  // 로고가 있으면 그것만 쓴다. 기본 아이콘(BRIGHT)을 maskable 로 함께 넣으면 안드로이드가 maskable 을 우선해
  // 학원 로고 대신 기본 아이콘을 홈 화면에 놓는다 — 고치려던 바로 그 증상이다.
  // 로고가 없을 때만 BRIGHT 기본 세 장(any 192·512 + maskable 512)을 쓴다.
  const icons: ManifestIcon[] = i.logoUrl
    ...
    : [{ src: `${root}logo/bright-icon-192.png`, sizes: '192x192', type: 'image/png' },
       { src: `${root}logo/bright-icon-512.png`, sizes: '512x512', type: 'image/png' },
       { src: `${root}logo/bright-icon-maskable-512.png`, sizes: '512x512', type: 'image/png', purpose: 'maskable' }];
...
export function appleTouchIcon(i: IdentityInput): string {
  return isPng(i.logoUrl) ? i.logoUrl! : `${absBase(i.base, i.origin)}logo/bright-icon-192.png`;
}
