...
// claim 이 잡을 수 있는 행인가? (0006 outbox_claim 의 WHERE 를 그대로)
async function claimable(id) {
  ...
  return { row: data, ok: ['queued', 'failed'].includes(data.status) && data.attempts < 5 && due };
}
const mkRow = (extra) => ({
...
{
  const r = await admin.from('outbox').insert(mkRow({ channel: 'alimtalk', template_code: 'NOTICE_NEW', params: { 학원: ctx.academyName, 제목: '가'.repeat(3000) } })).select('id').single();
  const id = r.data.id;
  let smsRow = null;
  for (let i = 1; i <= 6; i++) {
