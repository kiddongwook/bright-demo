...
// 할 것 5
await once('todos', { title: '독해 워크북 p.42–45' }, { class_id: cls['고2 B'], kind: 'homework', title: '독해 워크북 p.42–45', due_date: kst(2) });
await once('todos', { title: '단어시험 51~75' }, { class_id: cls['고2 B'], kind: 'exam', title: '단어시험 51~75', due_date: kst(2), notice_id: n3 });
const tDone = await once('todos', { title: '영작 1편 — My Summer Plan' }, { class_id: cls['고2 B'], kind: 'homework', title: '영작 1편 — My Summer Plan', due_date: kst(0) });
await sb.from('todo_done').upsert({ todo_id: tDone, student_id: st['김민수'] }, { ignoreDuplicates: true });
await once('todos', { title: '모의고사 기출 3회' }, { class_id: cls['고1 A'], kind: 'homework', title: '모의고사 기출 3회', due_date: kst(3) });
const tA = await once('todos', { title: '단어장 26~50 외우기' }, { class_id: cls['고1 A'], kind: 'homework', title: '단어장 26~50 외우기', due_date: kst(1) });
...
await once('inquiries', { topic: '다음 주 화요일 결석 예정' }, { student_id: st['박지훈'], asked_by: pJ, topic: '다음 주 화요일 결석 예정', body: '다음 주 화요일 지훈이 가족 행사로 결석 예정입니다. 보강 가능할까요?', answer: '어머님 안녕하세요 :) 그날은 휴원일이라 원래 수업이 없어요. 걱정 안 하셔도 됩니다. 즐거운 행사 되세요!', answered_by: dir, answered_at: new Date(Date.now() - 86400e3).toISOString() });
await once('inquiries', { topic: '레벨 테스트 언제' }, { student_id: st['최유나'], asked_by: pC, topic: '레벨 테스트 언제', body: '레벨 테스트는 언제 보나요?', answer: '다음 주 월요일 수업 끝나고 20분 정도 봐요. 따로 준비하실 건 없어요.', answered_by: dir, answered_at: new Date(Date.now() - 3 * 86400e3).toISOString() });
// 결석 2
await once('absence_requests', { student_id: st['최유나'], date: kst(3) }, { student_id: st['최유나'], requested_by: pC, date: kst(3), reason: '병원 진료' });
await once('absence_requests', { student_id: st['한지우'], date: kst(-5) }, { student_id: st['한지우'], requested_by: pH, date: kst(-5), reason: '가족 여행', status: 'confirmed', makeup_kind: 'saturday', makeup_at: new Date(Date.now() - 3 * 86400e3).toISOString(), decided_by: dir });
// 오늘 출결 (고2 B)
